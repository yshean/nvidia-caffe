...
# LD library path
ENV LD_LIBRARY_PATH /usr/local/cuda-6.5/lib64
...
# Install cuDNN v2
RUN wget https://s3.amazonaws.com/deep-learning-tools/cudnn-6.5-linux-x64-v2.tgz && \
    tar -zxvf cudnn-6.5-linux-x64-v2.tgz && \
    cd cudnn-6.5-linux-x64-v2 && \
    cp cudnn.h /usr/local/cuda/include && \
    cp libcudnn* /usr/local/cuda/lib
    cp cudnn.h /usr/local/cuda-6.5/include && \
    cp libcudnn* /usr/local/cuda-6.5/lib64

# Clone the Caffe repo 
RUN cd /opt && git clone https://github.com/NVIDIA/caffe.git
...
# Install python deps
RUN cd /opt/caffe && \
  (pip install -r python/requirements.txt; easy_install numpy; easy_install scipy; pip install -r python/requirements.txt) && \
  easy_install pillow
